...
time     = expData.jointPosMeas_SCOPE.time;
jointPos = [];
w_H_b    = [];

for k = 1:length(expData.jointPosMeas_SCOPE.signals)
    
    jointPos = [jointPos; squeeze(expData.jointPosMeas_SCOPE.signals(k).values)*pi/180]; %#ok<AGROW>
end

for k = 1:3
    
    basePos(:,k) = squeeze(expData.basePos_SCOPE.signals(k).values); %#ok<SAGROW>
    baseRot(:,k) = squeeze(expData.baseRot_SCOPE.signals(k).values)*pi/180; %#ok<SAGROW>
end

for k = 1:length(time)
    
    w_H_b_matr  = wbc.fromPosRpyToTransfMatrix([basePos(k,:), baseRot(k,:)]');
