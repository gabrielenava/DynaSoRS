function [] = idyn_setFloatingBase(KinDynModel,floatBaseLinkName)

    % IDYN_SETFLOATINGBASE sets the link that is used as floating base.
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT: [] = idyn_setFloatingBase(KinDynModel,floatBaseLinkName)
    %
    ...
    % check for errors
    if ~ack  
        error('[idyn_setFloatingBase]: unable to set the floating base link.')
    end  
end