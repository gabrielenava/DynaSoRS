function g = idyn_generalizedGravityForces(KinDynModel)

    % IDYN_GENERALIZEDGRAVITYFORCES retrieves the generalized gravity forces 
    %                               given the reduced model.
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  g = idyn_generalizedGravityForces(KinDynModel)
    %
    ...
    % check for errors
    if ~ack  
        error('[idyn_generalizedGravityForces]: unable to get the gravity forces from the reduced model.')
    end
