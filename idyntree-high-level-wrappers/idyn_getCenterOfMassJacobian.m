function J_CoM = idyn_getCenterOfMassJacobian(KinDynModel)

    % IDYN_GETCENTEROFMASSJACOBIAN retrieves the CoM jacobian.
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  J_CoM = idyn_getCenterOfMassJacobian(KinDynModel)
    %
    ...
    % check for errors
    if ~ack    
        error('[idyn_getCenterOfMassJacobian]: unable to get the CoM Jacobian from the reduced model.')
    end
