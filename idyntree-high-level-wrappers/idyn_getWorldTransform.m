function w_H_frame = idyn_getWorldTransform(KinDynModel,frameName)

    % IDYN_GETWORLDTRANSFORM gets the transformation matrix between a specified 
    ...
    %
    % FORMAT:  w_H_frame = idyn_getWorldTransform(KinDynModel,frameName)
    %
    ...
    if KinDynModel.DEBUG
        ...
        disp('[idyn_getWorldTransform]: debugging outputs...')
        
         % w_R_frame must be a valid rotation matrix
         if det(w_R_frame) < 0.9 || det(w_R_frame) > 1.1
            
             error('[idyn_getWorldTransform]: w_R_frame is not a valid rotation matrix.')
         end
