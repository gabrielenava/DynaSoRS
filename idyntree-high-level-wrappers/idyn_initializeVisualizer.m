function Visualizer = idyn_initializeVisualizer(KinDynModel,debugMode)

    % IDYN_INITIALIZEVISUALIZER opens the iDyntree visualizer and loads the reduced
    ...
    %
    % FORMAT:  Visualizer = idyn_initializeVisualizer(KinDynModel,debugMode)
    %
    ...
    % check for errors
    if ~ack   
        error('[idyn_initializeVisualizer]: unable to load the model in the visualizer.')
    end  
