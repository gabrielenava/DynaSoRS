function J_frame = idyn_getFrameFreeFloatingJacobian(KinDynModel,frameName)

    % IDYN_GETFRAMEFREEFLOATINGJACOBIAN gets the free floating jacobian of a 
    %                                   specified frame. 
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  J_frame = idyn_getFrameFreeFloatingJacobian(KinDynModel,frameName)
    %
    ...
    % check for errors
    if ~ack   
        error('[idyn_getFrameFreeFloatingJacobian]: unable to get the Jacobian from the reduced model.')
    end
