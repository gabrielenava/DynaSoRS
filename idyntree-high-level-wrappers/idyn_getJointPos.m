function jointPos = idyn_getJointPos(KinDynModel)

    % IDYN_GETJOINTPOS retrieves the joints configuration from the reduced model.
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  jointPos = idyn_getJointPos(KinDynModel)
    %
    ...
    % check for errors
    if ~ack   
        error('[idyn_getJointPos]: unable to retrieve the joint positions from the reduced model.')
    end
