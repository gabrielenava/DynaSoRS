function stateVel = idyn_getModelVel(KinDynModel)

    % IDYN_GETMODELVEL gets the joints and floating base velocities from the 
    %                  reduced model.
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  stateVel = idyn_getModelVel(KinDynModel)
    %
    ...
    % check for errors
    if ~ack  
        error('[idyn_getModelVel]: unable to retrieve the state velocities from the reduced model.')
    end
