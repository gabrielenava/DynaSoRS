function h = idyn_generalizedBiasForces(KinDynModel)

    % IDYN_GENERALIZEDBIASFORCES retrieves the generalized bias forces from 
    %                            the reduced model. 
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  h = idyn_generalizedBiasForces(KinDynModel)
    %
    ...
    % check for errors
    if ~ack
        error('[idyn_generalizedBiasForces]: unable to get the bias forces from the reduced model.')
    end
