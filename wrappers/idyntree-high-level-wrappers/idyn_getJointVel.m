function jointVel = idyn_getJointVel(KinDynModel)

    % IDYN_GETJOINTVEL retrieves joint velocities from the reduced model.
    %
    % This matlab function wraps a functionality of the iDyntree library.                     
    % For further info see also: http://wiki.icub.org/codyco/dox/html/idyntree/html/
    %
    % FORMAT:  jointVel = idyn_getJointVel(KinDynModel)
    %
    ...
    % check for errors
    if ~ack   
        error('[idyn_getJointVel]: unable to retrieve the joint velocities from the reduced model.')
    end
