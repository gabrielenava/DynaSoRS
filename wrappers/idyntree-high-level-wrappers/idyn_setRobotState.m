function [] = idyn_setRobotState(varargin)

    % IDYN_SETROBOTSTATE sets the system state. The system state is composed of:
    %                   
    ...
    %
    %         [] = idyn_setRobotState(KinDynModel,basePose,jointPos,baseVel,jointVel,gravAcc)
    %
    %         Fixed base system:
    %
    %         [] = idyn_setRobotState(KinDynModel,jointPos,jointVel,gravAcc)
    %
    ...
    switch nargin
        ...
        case 6
            ...
            if KinDynModel.DEBUG
                ...
                disp('[idyn_setRobotState]: debugging inputs...')
        
                % basePose must be a valid transformation matrix
                if size(basePose,1) ~= 4 || size(basePose,2) ~= 4
            
                    error('[idyn_setRobotState]: basePose is not a 4x4 matrix.')
                end
                ...
                for ii = 1:4
                    ...
                    if ii < 4
                        ...
                        if abs(basePose(4,ii)) > 0.0001 
                            ...
                            error('[idyn_setRobotState]: the last line of basePose is not [0,0,0,1].')
                        end
                    else
                ...
                if det(basePose(1:3,1:3)) < 0.9 || det(basePose(1:3,1:3)) > 1.1
                    ...
                    error('[idyn_setRobotState]: baseRotation is not a valid rotation matrix.')
                end
                ...
                if length(gravAcc) ~= 3
                     ...
                     error('[idyn_setRobotState]: the length of gravAcc vector is not 3.')
                end 
                
                % check base velocity vector size
                if length(baseVel) ~= 6
            
                     error('[idyn_setRobotState]: the length of baseVel is not 6.')
                end
                
                % check joints position vector size
                if length(jointPos) ~= KinDynModel.NDOF
            
                    error('[idyn_setRobotState]: the length of jointPos is not KinDynModel.NDOF')
                end
                
                % check joints velocity vector size
                if length(jointVel) ~= KinDynModel.NDOF
            
                    error('[idyn_setRobotState]: the length of jointVel is not KinDynModel.NDOF')
                end
                
                disp('[idyn_setRobotState]: done.')     
            end
        ...
        otherwise        
            error('[idyn_setRobotState]: wrong number of inputs.')
    end
    ...
    % check for errors
    if ~ack   
        error('[idyn_setRobotState]: unable to set the robot state.')
    end  
end