function [] = idyn_updateVisualizer(Visualizer,KinDynModel,jointPos,basePose)

    % IDYN_UPDATEVISUALIZER updates the iDyntree visualizer with the current
    ...
    %
    % FORMAT: [] = idyn_updateVisualizer(Visualizer,KinDynModel,jointPos,basePose)
    %
    ...
    if Visualizer.DEBUG
        ...
        disp('[idyn_updateVisualizer]: debugging inputs...')
        
        % basePose must be a valid transformation matrix
        if size(basePose,1) ~= 4 || size(basePose,2) ~= 4
            
            error('[idyn_updateVisualizer]: basePose is not a 4x4 matrix.')
        end
        ...
        for ii = 1:4
            ...
            if ii < 4
                ...
                if abs(basePose(4,ii)) > 0.0001 
                    ...
                    error('[idyn_updateVisualizer]: the last line of basePose is not [0,0,0,1].')
                end
            else
        ...
        if det(basePose(1:3,1:3)) < 0.9 || det(basePose(1:3,1:3)) > 1.1
            ...
            error('[idyn_updateVisualizer]: baseRotation is not a valid rotation matrix.')
        end
        ...
        if length(jointPos) ~= KinDynModel.NDOF
            ...
            error('[idyn_updateVisualizer]: the length of jointPos is not KinDynModel.NDOF')
        end
            
        disp('[idyn_updateVisualizer]: done.')     
    end
    ...
    % check for errors
    if ~ack    
        error('[idyn_updateVisualizer]: unable to update the visualizer.')
    end
